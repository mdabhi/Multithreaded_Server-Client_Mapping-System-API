...
static int printRoad(int* route, int hops, char* path, int* size)
{
	...
	sz_c = snprintf(&path[sz], sz_r, "%d|", hops - 1);
	if(sz_c > sz_r) {
		printf("ERROR: Not enough memory for sending route\n");
		return -1;
	} else {
		sz += sz_c;
	}
		...
		while (pedge) {
			if(pedge->dest == route[i+1]) {
				if(i == 0) {
					...
					sz_c = snprintf(&path[sz], sz_r, "%s|%0.2f|%0.2f|",
							road.roadname, road.speed, road.distance);
					if(sz_c > sz_r) {
						printf("ERROR: Not enough memory for sending route\n");
						return -1;
...
static int dk(int src, int des, char* path, int* size)
{
	...
	ret = printRoad(route, hops, path, size);
...
int trip(char* vertex1, char* vertex2, char* path, int* size)
{
	int src = -1;
	int dest = -1;
